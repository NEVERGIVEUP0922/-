...
abstract class ThinkOauth
{
    ...
    protected function config()
    {
        ...
        if (empty($config['CALLBACK']))
            $this->Callback = 'http://'.C('WEB_ROOT_DOMAIN').U('Home/Account/'.strtolower($this->Type).'Return');
        else
     ...
     */
    protected function http($url, $params, $method = 'GET', $multi = false, $header = array())
    {
    	$opts = [];
        /* 根据请求类型设置特定参数 */
        switch (strtoupper($method)) {
            ...
            case 'POST':
                //判断是否传输文件
                $params = $multi ? $this->build_http_query_multi($params) : http_build_query($params);
	            if( $multi ){
...
//	            exit();
	...
	public function build_http_query_multi($params) {
		...
		$pairs = array();
		//----------------
		$this->boundary = $boundary = uniqid('5eE2ES');
		$MPboundary = '--'.$boundary;
		...
		foreach ($params as $parameter => $value) {
			...
			if( in_array($parameter, array('pic', 'image')) && $value{0} == '@' ) {
				...
				$multipartbody .= "Content-Type: image/".$fileExt."\r\n\r\n";
