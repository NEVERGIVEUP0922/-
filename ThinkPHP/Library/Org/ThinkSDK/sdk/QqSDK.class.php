...
class QqSDK extends ThinkOauth
{
	 ...
	 * @var srting
	 */
	protected $Authorize = 'scope=get_user_info,add_share,get_info,add_t';
	...
	public function getWb()
	{
		...
		if (isset($data['openid']))
			return $data['openid'];
		elseif ($data['access_token']) {
			$data = $this->http($this->url('user/get_info'), array('access_token' => $data['access_token']));
			$data = json_decode(trim(substr($data, 9), " );\n"), true);
			if (isset($data['openid']))
				return $data['openid'];
			else
				throw new Exception("获取微博出错：{$data['error_description']}");
		} else {
			throw new Exception('没有获取到微博用户信息！');
		}
	}
