 ...
 * @return string
 */
function U($url='',$vars='',$suffix=true,$domain=false, $params__ = false ) {
    // 解析URL
    $info   =  parse_url($url);
    ...
    if( $params__ ){
    	return [
    		'cle_url'=>$url,
			'model'=>strtolower($varArr[$varModule]),
			'controller'=>strtolower($varArr[$varController]),
			'action'=>strtolower($varArr[$varAction]),
			'mca'=>'',
			'params'=>$parsmArr,
		];
	}
