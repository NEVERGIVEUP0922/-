...
class OrderController extends EESController
{
	...
	public function storeInfoSyncResult( $result )
	{
		...
		if ( isset( $result[ 'success' ] ) ) {
			foreach ( $result[ 'success' ] as $k => $value ) {
				...
				$res = M('user_handle_history','sys_')->where([ 'index'=>$value['order_no'] ])->data(['sync_status'=>1,'sync_msg'=>'同步成功'])->save();
				if($res=== false){
					Log::write( '价格信息等同步订单错误: 商城订单' . $value[ 'order_no' ] . ' 同步纪录失败!');
					continue;
				}
		...
		if ( isset( $result[ 'error' ] ) ) {
			foreach ( $result[ 'error' ] as $k => $value ) {
				$orderInfo = M( 'order' )->where( [ 'order_sn' =>  $value[ 'order_sn' ]] )->find();
				if ( !$orderInfo ) continue;
				M()->startTrans();
				//订单已删除
				if( $value['del'] === 1 ){
					$mDel=M('user_handle_history','sys_')->where([ 'index'=>$value['order_sn'] ])->data(['sync_status'=>3,'sync_msg'=>'订单删除'])->save();
					
					if( $mDel !== false){
						$history=M('user_handle_history','sys_')->field('data_old')->where([ 'index'=>$value['order_sn'] ])->find();
						if($history){
							$oldData=json_decode($history['data_old'],true);
							$orderInfo1['total']=$oldData['data']['list'][0]['total'];
							$orderInfo1['order_sn_sale']=$oldData['data']['list'][0]['order_sn_sale'];
							$order_result=M('order')->where(['order_sn'=>$value['order_sn']])->save($orderInfo1);
							if($order_result){
								foreach ($oldData['data']['list'][0]['goodsList'] as $good_value){
									$orderGood['p_price_true']=$good_value['p_price_true'];
									$orderGood['p_sign_sale']=$good_value['p_sign_sale'];
									$orderGood['pay_subtotal']=$good_value['pay_subtotal'];
									$good_result=M('order_goods')->where(['id'=>$good_value['id']])->save($orderGood);
									if(!$good_result){
										M()->rollback();
										Log::write( '价格信息等同步订单错误: 商城订单' . $value[ 'order_sn' ] . ' 同步纪录失败!');
										continue;
									}
								}
							}else{
					...
					$mErr=M('user_handle_history','sys_')->where([ 'index'=>$value['order_sn'] ])->data(['sync_status'=>2,'sync_msg'=> str_replace('Error: ','',$value[ 'msg' ])])->save();
					if($mErr){
						$history=M('user_handle_history','sys_')->field('data_old')->where([ 'index'=>$value['order_sn'] ])->find();
	...
	public function addInfoOrder( $orderNo )
	{
		//查询订单数据
		...
		$baseData = [
			...
			'address'       => $shAddr,
			'inremark'      =>$inremark,
			'custorderno'   =>$orderInfo['order_sn_sale'],
		];
		...
		foreach ( $orderInfo[ 'goods' ] as $k => $v ) {
			...
			if($v['rule_info']){
                ...
                if(!isset($rule['return_price'])){
                    $rule['return_price']=0.00;
                }
			}else{
				$rule['return_price']=0;
			}
