...
use Common\Model\MessageDetailModel;


/**
 * 商城首页Controller
 */
class IndexController extends BasketController{
//	/**
//	 * 首页
//	 */
...
//    }
    ...
    {
        $mq = MQ::getInstance();
//        de($mq->writeMsg(0,true,0,0,10415,0,'通知312', '这是一条通知123122132131'));
        de( $mq->getAllMsgList(10415,true,1,3) );
//        de( $mq->setMsgDel(10415,true,2) );
//        $model = new MessageDetailModel();
//        de($model->getMsgListByType( 1, 16,false ));
    }

    /**
     * 用来做测试用
     */
    public function allowIpSet(){
        $request=I('get.');
        if($request['token']!='!!!') die();
        $key=md5('allowIpSet');
        $ip=get_client_ip();
        $has=S($key);
