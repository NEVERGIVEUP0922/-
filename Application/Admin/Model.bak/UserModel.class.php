...
class UserModel extends BaseModel
{
    ...
    public function adminAction($request){
        if($request['action']=='delete') return $this->adminDelete(['uid'=>$request['uid']]);
        ...
        $request['qq']=$request['qq'];
        $request['wechat']=$request['wechat'];
        ...
        if($request['action']=='add'){
            ...
            if(M('user','sys_')->add($request)) $result=1;
        }else if($request['action']=='edit'){
            if(isset($request['user_name'])) unset($request['user_name']);
            if(M('user','sys_')->where(['uid'=>$request['uid']])->save($request)!==false) $result=1;
        }
...
//            }
    ...
    public function departmentDataPower($power_type,$adminDepartmentId,$where,$key){
        ...
        if($power){
            ...
            return ['error'=>0,'data'=>['where'=>$where,'must_where'=>$must_where]];
        }else{
