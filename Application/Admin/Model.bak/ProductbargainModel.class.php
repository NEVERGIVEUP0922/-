...
class ProductbargainModel extends BaseModel
{
     ...
     *
     */
    public function productBargainList($where='',$page='',$pageSize='',$order=''){
        $must_where=[];
        ...
        if($data){
            ...
            foreach($data as $k=>$v){
                $one_pass=$result_pass['data']['list'][$v['p_id']];
        ...
        $pIdList=$this->baseListType(M('product'),array_unique($p_id),'id');
        $pIdData=$pIdList['data']['list'];
        $sysUserList=$this->baseListType(M('user','sys_'),array_unique($sys_uid),'uid');
        ...
        foreach($list['data']['list'] as $k=>$v){
            ...
            $one=[
                ...
                'fitemno'=>$pIdData[$v['p_id']]['fitemno'],
    ...
    public function oneProductBargainAction($data){
        ...
        if(!M('product')->where(['id'=>$data['p_id']])->find()){ return ['error'=>1,'msg'=>'产品信息错误']; }
        if(!M('user')->where(['id'=>$data['uid']])->find()){ return ['error'=>1,'msg'=>'客户信息错误']; }

        $oneProduct=(new \Admin\Model\ProductModel)->productList(['id'=>$data['p_id']]);
...
//        }
        ...
        $bargain=M('user_product_bargain');
        $bargain->startTrans();
        $result1=$bargain->where(['uid'=>$data['uid'],'p_id'=>$data['p_id']])->save(['is_del'=>1]);
        $result2=$bargain->add($data);
        if($result1!==false&&$result2){
            $bargain->commit();
            return ['error'=>0,'msg'=>$msg,'id'=>$result2];
        }else{
            $bargain->rollback();
    ...
    public function productDiscountPriceInfo($cusId,$pId_arr,$data_argument=''){
        ...
        if(!$data_argument){
            ...
            foreach($data_argument as $k=>$v){
                $discountData[$v['p_id']]=$v;
            }
        }
        $productResult=(new ProductModel())->productMinPriceFstcb($pId_arr);
    ...
    public function customerProductSampleActions($request){
        if(!$request||!is_array($request)) return ['error'=>1,'msg'=>'参数错误'];
        $result=['error'=>0];
    ...
    public function customerProductSampleAction($one,$sys_uid='',$is_delete=false){
        ...
        $result=$m->field('uid,pid,sys_uid')->add($one);
    ...
    public function customerProductSampleList($where='',$page='',$pageSize='',$order='id desc',$field=''){
        ...
        $product=$this->baseListType(M('product'),$pid_arr,'id','id,p_sign');
