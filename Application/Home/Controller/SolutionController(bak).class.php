...
//		print_e($last_ff);
		...
		//print_e($_SERVER);	print_e(strrchr($local_url, '/'));	print_e(trim(strrchr($local_url, '/'),'/'));
		//print_e(strpos($local_ff,'.'));	print_e(substr($local_ff,0,strpos($local_ff,'.')));
...
//		$msg_type = ['audit','conversing','conversed'];//审核动作,接洽动作(洽谈内容),被洽谈动作(是否接受洽谈)
		$uid  = session('userId');
		$data = D('SolutionNotify')->get_msg($uid);
...
//		$_model  = new SolutionProviderModel();
	...
	public function solCollectList()
	{
		...
		$pageNum = 4;
//		$limit	 = ($page-1)*$pageSize.",".$pageNum;
		/*$field= ['C.id,C.uid,C.sol_uid,C.sol_id,C.sol_sn,C.sol_type,C.types_id,C.create_time,C.collected,P.jl_self,P.pro_sn,P.conver_image,P.pro_name,P.synopsis,P.pcba,P.browse,P.collection,P.volume,P.pubtime,D.desir_sn,D.desir_name,D.types'];*/
		$field1 = ['*,C.id,P.uid,C.create_time'];
		$field2 = ['*,C.id,D.uid,C.create_time'];
		$where = ['C.uid='.$uid,'C.collected=1'];
		$temp1 = M('solution_collect')->alias('C')
			->join('right join dx_solution_provider as P on P.pro_sn = C.sol_sn')
			->join('left join dx_solution_vip as V on V.uid = P.uid')
			->field($field1)->where($where)->order('C.create_time desc')->select();

		$temp2 = M('solution_collect')->alias('C')
			->join('right join dx_solution_desired  as D on D.desir_sn= C.sol_sn')
			->join('left join dx_solution_vip as V on V.uid = D.uid')
			->field($field2)->where($where)->order('C.create_time desc')->select();
		$res = $this->deletNull($temp1,$temp2,true);//合并数组并去除空值
		if(!empty($res)){
			$count = count($res);//分页
		}
...
//		if((int)$request['types'] != 999 ){
		...
		if($request['type1'] == null && $request['type2'] == null){//表示默认的供应方案,
			...
			$field  = ['*,P.id,P.uid'];//字段.
//			$field  = ['P.id,P.uid,P.jl_self,P.pro_sn,P.conver_image,P.imgs,P.pro_name,P.synopsis,P.types,P.pcba,P.pcba_list,P.browse,P.collection,P.volume,P.pubtime,V.nick_name,V.nick_picture'];//字段.
...
//			getsql();print_e($temp);
			...
			$field  = ['*,D.id,D.uid'];//字段
//			$field  = ['D.id,D.uid,D.jl_self,D.desir_sn,D.desir_name,D.desir_desc,D.delivery,D.budget,D.publish_status,D.types,D.browse,D.collection,D.pubtime,V.nick_name,V.nick_picture'];//字段
...
//			getsql();print_e($temp);die;
	...
	public function sol_page($model,$condition,$page,$pageSize,$field,$sork,$desc,$sol_type=1)
	{
		if($sol_type==1){//供应方案
			...
			$temp = D($model)->alias('P')->join("left join dx_solution_vip as V on P.uid = V.uid")->order($sork.' '.$desc)->where($condition)->limit(($page-1)*$pageSize,$pageSize)->field($field)->select();
//			getsql();print_e($temp);die;
...
//------方案管理中心------------------------------------------------------------------------------------------------------
	//	商业协议书
	public function business_protocol(){
		$file = './Public/Home/Solution/business_rule.txt'; //先读取文件
...
//			if($check_vip!==2)$this->ajaxReturnStatus(1023,'您目前还不是会员');
			...
			/*限制(非会员)提交需求方案次数*/
			$action_limit = new SolutionModel;
			$info = $action_limit->action_limit($user_IP,10,20);
	...
	//方案图片处理-----oss
	public function descImg()
	{
		//Home/Upload控制器descImg方法
