...
//                    }
                   ...
                   // $wallet=new WalletController();
                    //$wa_intergral=$wallet->returnIntegral([$v['id']]);      
                    ...
                    $add_data_goods[] = [
                        ...
                        //'wa_intergral'=>$wa_intergral?json_encode($wa_intergral[$v['id']]):"",
            ...
            //下单
            M('order')->startTrans();
           if($data["consume_integral"]>0){
                $inter_add["user_id"]=$user_id;
                $inter_add["integral_amount"]=$data["consume_integral"];
                $inter_add["create_time"]=date("Y-m-d H:s:i");
                $inter_add["status"]=1;
                $inter_add["order_sn"]=$order_sn."抵扣";
                $inter_add["d_status"]=1;
                M("intergral_all","wa_")->add($inter_add);
            }
            //            //下单成功,定单积分
        ...
        $allintegral=$this->returnIntergral($user_id);
        $this->assign('allintegral',$allintegral);
  ...
  //积分抵扣--获取抵扣积分
    public function orderIntegral($user_id=''){
        $request=I('post.');
        $default=new \Home\Controller\DefaultController();
        $integral=$default->returnIntergral($user_id);
        //$integral_Proportion=0.01;
        return $integral;//number_format(($integral>0?$integral:0.00)*$integral_Proportion,2);
    }
        ...
        if(!$info){
            ...
            if($hysave===false){
                ...
                $res=$order->where(['order_sn'=>$order_sn])->save($ordersave);
              	if($ordersave["order_status"]==3&&0){
                    $u_all=M("order")->where(["order_sn"=>$order_sn])->find();
				    $o_all=M("order_goods")->where(["order_sn"=>$order_sn])->select();
				    foreach ($o_all as $o_value){
				        if($o_value["wa_intergral"]){
                         
                            $o_price=($o_value["p_num"]-$o_value["knot_num"])*$o_value["p_price_true"];
                            $this->erpIntergral(json_decode($o_value["wa_intergral"],true),$order_sn,'',$u_all["user_id"],$o_price);
                          //	file_put_contents("666.txt","2223232332");
                            file_put_contents("1122.txt",$o_value["wa_intergral"]."---".$change['order_no'].'---'.$order_sn."---".$o_price);
                        }
                    }
